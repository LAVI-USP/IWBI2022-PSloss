...
def model_forward(model, img_ld, red_factor, batch_size):
    ...
    # Normalize image
    img_ld = scale(img_ld, red_factor=red_factor, vmin=48, vmax=2500.0)    
    ...
    # Normalize image (Inv)
    rst_img = de_scale(rst_img, vmax=2500.0)
    
    return rst_img

def test(model, path_data, path2write, red_factor, mAsLowDose, batch_size, model_name):
    ...
    for file_name in tqdm(file_names):
        ...
        folder_name = path2write + 'DBT_DL_' + model_name + '_' + file_name.split('/')[-2] 
        file2write_name = 'DL_' + model_name + '_' + file_name.split('/') [-1]
...
if __name__ == '__main__':
    ...
    path_data = ""
    path_models = "final_models/rlz_{}/{}/".format(rlz,typ)
    path2write = ""
    
    makedir(path2write)
    
    path_final_generator = path_models + "HResNet_{}-{}mAs.pth".format(model_name,mAsLowDose)
    ...
    _= load_model(model,path_final_model=path_final_generator)
    
    print("Running test on {}.".format(device))
    
    test(model, path_data, path2write, red_factor, mAsLowDose, batch_size, model_name)
